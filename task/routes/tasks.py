...
from task.schemas import TaskIn, UpdateTask, Done, Verify
from task.db.models import TaskDB, UserDB
from task.db.main import AsyncSession, get_db
from task.jwt import get_current_user
from task.checker import RoleChecker as c
...
async def all_tasks(db:AsyncSession = Depends(get_db), user = Depends(get_current_user), role = Depends(c.check_role)):
    uid = user["uid"]
    try:
